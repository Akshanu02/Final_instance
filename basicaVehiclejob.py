...
#https://github.com/awslabs/aws-iot-device-client/blob/main/sample-job-handlers/download-file.sh

parser = argparse.ArgumentParser(description='An AWS IoT Device Management Jobs agent implementation')
parser.add_argument('-e', '--endpoint', required=True)
parser.add_argument('-c', '--cert', required=False, default = './certificate1.pem')
parser.add_argument('-k', '--key', required=False, default = './privatekey1.key')
parser.add_argument('-r', '--rootca', required=False, default = './rootCA.pem')
...
class DeviceJobAgent:
    ...
    def executeJob(self, execution):
        ...
        for step in execution['jobDocument']['steps']:
            subprocess.run(['bash', './'+step['action']['input']['path']+step['action']['input']['handler'], step['action']['runAsUser']] + step['action']['input']['args'])
    ...
    def startNextJob(self, client, userdata, message):
        ...
        if 'execution' in job:
            ...
            self.updateJobStatus(statusDetails, execution['jobId'], execution['versionNumber'], executionNumber)
            self._jobCompleted = True
        else:
    ...
    def hasJobCompleted(self):
        return self._jobCompleted
    ...
    def init(self):
        ...
        self._iotClient.publish(self._jobTopicBase+'/start-next', payload=json.dumps({'clientToken': self._token}), QoS=1)
